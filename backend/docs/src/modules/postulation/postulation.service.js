...
async applyToJob(studentId, jobId) {
    ...
    const applicantIds = (job.applications || []).map(app =>
        String(app.applicant?._id || app.applicant)
    );

    if (applicantIds.includes(String(studentId))) {
        throw { status: 409, message: "Ya te has postulado a esta oferta." };
    }

    return await jobRepository.addApplicant(jobId, studentId);
}
async acceptApplicant(employerId, jobId, applicationId) {
    const job = await jobRepository.findById(jobId);
    ...
    const application = job.applications.find(app =>
        String(app._id) === String(applicationId)
    );

    if (!application) {
        throw { status: 400, message: "La postulación no pertenece a esta oferta." };
    }

    const updatedJob = await jobRepository.acceptApplicant(
        jobId,
